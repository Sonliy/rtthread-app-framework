...
/* RS485 口（UART1，对上位机）*/
static volatile uint16_t g_rs485_rx_head, g_rs485_rx_tail;
static uint8_t  g_rs485_rx_buf[COM_PC_BUF_SIZE];

/* TTL 口（UART0，对 DSP）*/
static volatile uint16_t g_dsp_rx_head, g_dsp_rx_tail;
static uint8_t  g_dsp_rx_buf[COM_PC_BUF_SIZE];

void UART_RS485_RX_ISR(uint8_t byte)
{
    uint16_t next = (g_rs485_rx_head + 1) % COM_PC_BUF_SIZE;
    if (next != g_rs485_rx_tail) { g_rs485_rx_buf[g_rs485_rx_head] = byte; g_rs485_rx_head = next; }
}

void UART_DSP_RX_ISR(uint8_t byte)
{
    uint16_t next = (g_dsp_rx_head + 1) % COM_PC_BUF_SIZE;
    if (next != g_dsp_rx_tail) { g_dsp_rx_buf[g_dsp_rx_head] = byte; g_dsp_rx_head = next; }
}

static uint16_t uart_ring_rs485_read(uint8_t *dst, uint16_t max)
{
    uint16_t cnt = 0;
    while (cnt < max && g_rs485_rx_tail != g_rs485_rx_head)
        { dst[cnt++] = g_rs485_rx_buf[g_rs485_rx_tail]; g_rs485_rx_tail = (g_rs485_rx_tail + 1) % COM_PC_BUF_SIZE; }
    return cnt;
}

static uint16_t uart_ring_dsp_read(uint8_t *dst, uint16_t max)
{
    uint16_t cnt = 0;
    while (cnt < max && g_dsp_rx_tail != g_dsp_rx_head)
        { dst[cnt++] = g_dsp_rx_buf[g_dsp_rx_tail]; g_dsp_rx_tail = (g_dsp_rx_tail + 1) % COM_PC_BUF_SIZE; }
    return cnt;
}
